...
- После запуска остаётся открытой небольшая палитра: можно менять выделение и повторно запускать crop без повторного запуска JSX.
...
Для обычного запуска через меню ScriptUI Panel не требуется: текущая версия открывает постоянную modeless-палитру.
...
3. Нажать `Crop...` в палитре.
4. Выбрать режим анализа и выполнить crop.
5. После отчёта изменить выделение и снова нажать `Crop...`.
6. Закрыть палитру системным крестиком или кнопкой `Close`.
...
- Добавлена постоянная launcher-палитра для повторных запусков с новым выделением; закрывается крестиком или кнопкой `Close`.
