...
Во время обработки внутри постоянного окна настроек появляется встроенная секция прогресса с кнопкой `Stop analysis`. Отмена проверяется между отдельными alpha samples, в том числе внутри анализа одного кадра. После завершения прохода секция скрывается. Если остановлен apply-pass, уже обработанные композиции остаются изменёнными, но весь проход находится в одном Undo Group.
