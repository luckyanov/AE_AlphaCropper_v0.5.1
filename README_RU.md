...
- При расчёте bounds считает Layer Opacity равной 100%, не изменяя и не удаляя исходные ключи или expressions.
- Даже в стандартном Current Frame автоматически расширяет анализ до всего timeline, если обнаружена анимация, способная менять геометрические bounds.
...
- После каждого Crop остаётся открытым полное окно настроек: можно менять выделение или параметры и повторно запускать crop без повторного запуска JSX.
- Явная кнопка закрытия называется `Exit`; завершение Crop не закрывает интерфейс скрипта.
- Если опциональное центрирование Anchor Point небезопасно для отдельного usage, применяется обычная компенсация якоря с предупреждением вместо пропуска всей source composition.
...
Скопировать JSX в папку Scripts установленной версии After Effects без номера версии в имени, например:

```text
C:\Program Files\Adobe\Adobe After Effects 2025\Support Files\Scripts\AlphaSmartCropper.jsx
```

после чего перезапустить AE.

Для обычного запуска через меню ScriptUI Panel не требуется: текущая версия открывает постоянную modeless-палитру с полными настройками.

---

## Базовое использование

1. Запустить `AlphaSmartCropper_v0.5.2.jsx` или установленный `AlphaSmartCropper.jsx`.
...
5. Закрыть постоянное окно настроек системным крестиком или кнопкой `Exit`.
...
- `Current Frame` — один текущий кадр для статичного или opacity-only содержимого; при геометрически значимой анимации автоматически анализирует весь timeline с шагом 1.
...
По умолчанию выбран `Current frame`. Для статичного или только opacity-анимированного содержимого он проверяет один кадр. Если Position, Scale, Rotation, Shape/Text properties, In/Out или другая визуально значимая анимация могут менять bounds, режим автоматически расширяется до всего source timeline с `Frame step = 1` и берёт union максимальных ненулевых пикселей.
...
Для статичного содержимого проверяется только текущий source frame. Если консервативный temporal-анализ обнаруживает анимацию, способную изменить видимые bounds, скрипт автоматически проверяет весь timeline покадрово. Поэтому анимированные Position/Scale/Rotation и другие визуальные свойства учитываются по максимальному union bounds и не должны обрезаться по одному кадру.

Layer Opacity исключена из этой классификации и во время alpha scan временно вычисляется как `100%`. Исходные ключи и expression не удаляются: скрипт накладывает временное выражение `100` на Layer Opacity во всей вложенной ветке и гарантированно восстанавливает прежнее состояние после анализа.
...
Встроенная секция прогресса всегда отображается внутри постоянного окна настроек. Начальное состояние — `Ready — 0%`; во время обработки показывается текущий процент и включается `Stop analysis`; после успеха остаётся `Complete — 100%`, после отмены — `Stopped`. Отмена проверяется между отдельными alpha samples, в том числе внутри анализа одного кадра. Если остановлен apply-pass, уже обработанные композиции остаются изменёнными, но весь проход находится в одном Undo Group.
...
10. Опциональное центрирование Anchor Point через Position намеренно ограничено безопасными 2D usages со статическими Scale/Rotation и без Collapse Transformations. Небезопасный usage получает обычную компенсацию якоря и предупреждение, но больше не блокирует Crop всей source composition.
11. Режим `Opacity = 100%` относится к Layer Transform Opacity. Mask Opacity, Shape Fill/Stroke Opacity и эффекты, изменяющие alpha, продолжают учитываться как часть финального рендера.
...
- Встроенная секция прогресса теперь постоянно отображается в основном окне настроек.
- Добавлены явные состояния `Ready — 0%`, текущий процент, `Complete — 100%` и `Stopped`.
- `Stop analysis` выключена в состоянии ожидания и включается только во время обработки.

## 0.5.1

- Отдельная modeless-палитра прогресса удалена: шкала и `Stop analysis` встроены в постоянное окно настроек, а секция скрывается до показа отчёта.
...
- Отдельная launcher-палитра заменена постоянным полным окном настроек: Crop оставляет его открытым, а `Exit` закрывает.
- Исправлена ошибочная классификация отсутствующего Layer Transform Skew как анимированного Skew.
- Небезопасное опциональное центрирование Anchor Point теперь отключается для конкретного usage и не пропускает всю source composition.
- Layer Opacity исключена из bounds: во время анализа она безопасно переопределяется значением 100% и затем восстанавливается.
- Current Frame автоматически расширяется до полного покадрового анализа при геометрически значимой анимации.
